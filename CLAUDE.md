...
- [x] **31.** Armar ~10 láminas para 10 minutos, una idea por lámina, siguiendo la secuencia: qué está pasando → por qué ocurre → qué implica para el negocio → qué recomendamos. *(Hecho: `docs/presentacion.pdf` y `docs/presentacion.pptx` editable, que queda como plan B. **La defensa se hace con el informe narrativo `docs/informe_narrativo.html` y después una demo del dashboard.** Todo se regenera con `python src/informe/generar.py`.)*
- [ ] **32.** Repasar juntos el banco de preguntas de más abajo hasta que **los dos** respondan las doce sin dudar. *(En `docs/guion_defensa.md`, punto 6, dice quién contesta cada una y qué se muestra en pantalla. Las preguntas 11 y 12 van de memoria.)*
- [ ] **33.** Ensayo cruzado: una pasada completa con los roles invertidos (Claudio hace los bloques A, C, E y G; Matías hace B, D y F). Si alguno se traba, esa parte no está entendida. *(Reparto, tiempos y diálogos en `docs/guion_defensa.md`.)*
- [ ] **34.** Ensayo final con cronómetro, 10 minutos reales, con el HTML y el dashboard en el computador de la presentación. Si se pasa, se corta, no se habla más rápido. *(El orden de corte está en `docs/guion_defensa.md`: primero los KPIs del capítulo 1, después EE.UU./Japón del capítulo 3, después el paso 1 de la demo. Nunca se cortan la auditoría, los íconos 53/21, las tres recomendaciones ni el límite.)*
...
10. Si histórico tiene la mejor nota, ¿por qué no lo recomiendan? *(Porque tiene el peor retorno: 1,16×. Ojo con la precisión: es la mejor nota entre las películas con datos financieros; en el catálogo completo, documental (7,01) lo supera. Recomendarlo sería recomendar prestigio de catálogo y no un caso de negocio. Lo decimos con esa palabra en el informe.)*
